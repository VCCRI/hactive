...
1. Hactive can only run on a Apple Mac computer since it is build with Xcode. Download Xcode from the app store on your Mac (~12gb).
...
This isn't so much a problem but it's important to call out: The iOS simulators on your computer don't come with any dummy data. This means that although the application will technically work using one of the inbuilt simulators, you won't get the full Hactive experience without any data. You'll see that the list of 'past workouts' will be empty. This is why we recommend using an actual iPhone as Apple watch. The Apple watch can collect data from your workouts and only a real iPhone can read this data since it is all stored in the Healthkit app on the iPhone (more on this later).
